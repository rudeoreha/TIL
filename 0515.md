...
## ✔️ forEach()
`forEach()` 는 배열에 활용이 가능한 메서드로, 파라미터로 주어진 함수를 배열 요소 각각에 대해 실행하는 메서드이다. `map()` 메서드와 거의 비슷하지만 차이점은 따로 return값이 없다는 점이다.

```js
const numbers = [12, 23, 32, 47, 565];
...
* 각 요소에 대해 실항할 함수. 다음 세 가지 매개변수를 받는다.
    ...
    \- `thisArg` : `callback` 을 실행할 때 `this` 로 사용할 값
...
//JavaScript에서는 "0"과 "음수" 모두 숫자형 데이터로 취급됩니다. 이들 값이 falsy한 값으로 취급되지 않는 이유는 숫자형 데이터 자체가 "true"나 "false"와 같은 불리언 값과는 다른 데이터 타입이기 때문입니다.

//따라서, "!n" 구문에서 "n"이 음수인 경우에는 해당 구문이 "false"를 반환합니다. 이는 "n"이 falsy한 값이 아니기 때문입니다.

//그러므로 "_.take" 함수에서도 "n"이 음수인 경우, 배열의 범위를 벗어난 것으로 판단하고 전체 배열을 반환해야 합니다.
};
...
_.drop = function (arr, n) {
  return _.slice(arr, n);// TODO: 여기에 코드를 작성합니다.
};
...
_.each = function (collection, iteratee) {
  // TODO: 여기에 코드를 작성합니다.
    // iteratee 함수가 입력값으로 들어옵니다.
  // 얘는 배열의 각 요소를 인자로 넣고 실행할 함수
  // 먼저 배열인지 아닌지 검사해서 배열이라면
  // 배열을 순회해서 iteratee를 실행하는 코드를 작성
...
_.indexOf = function (arr, target) {
  ...
  let result = -1;
// each를 사용해서 모든 배열을 순회
  // 배열의 요소를 검사하는데
  // 해당 요소가 target과 일치한다면
  // 해당 요소가 위치한 인덱스를 result에 할당
  // result를 반환
...
_.reject = function (arr, test) {
  ...
  // for 문 혹은 each를 사용해서 순회한뒤
  // test(배열의 요소) ==> false ==> result.push(배열의 요소)
  return result;
};
...
_.uniq = function (arr) {
  ...
  // 검증 단계를 하나 거칠거에요
  // result 배열에 해당 요소가 이미 존재한다면 걔는 포함하지마
  // indexOf
  // [1, 2, 2, 3, 4, 5, 5]
...
_.pluck = function (arr, keyOrIdx) {
  ...
  // TODO: 여기에 코드를 작성합니다.
   // _.pluck([[1,2], [2,3], [4,2]] , 1)
  // [[1,2], [2,3], [4,2]] 이 배열의 요소에서 1번째 인덱스들만 모아서 배열로 만들어 반환
  // [2, 3, 2]
  // _.pluck([{name: 은혜, gender: female},{name: 홍식, gender: female},{name: 아윤, gender: female}] , name)
  // [은혜, 홍식, 아윤]
  // 배열의 모든 요소를 순회한 뒤 []로 특정 인덱스 혹은 키를 참조해
  // 해당 값만 모은 새로운 배열을 반환
  // 객체의 특정 프로퍼티, 배열의 특정 인덱스 ==> 조회할 때 (),{}, []
